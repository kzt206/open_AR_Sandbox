...
try:
    ...
    logger.warning('dependencies not found for depthcamera D455 to work. Check installation and try again', exc_info=True)


# %%
class D455:
    """
    control class for the depthcamera D455 based on the Python wrappers of the Intel RealSense.
    Init the depthcamera D455 and provides a method that returns the scanned depth image as numpy array.
    ...
    def _update_frames(self):
        ...
        while True:
            ...
            self._depth = np.flipud(np.asanyarray(depth_frame.get_data()))   # up side down
            self._color = np.flipud(np.asanyarray(color_frame.get_data()))
            self._ir = np.flipud(np.asanyarray(infrared_frame.get_data()))
